        ...
        <div class="col-xs-12 col-sm-offset-3 col-sm-6">
            <div class="panel panel-default">
                <div class="panel-heading c-list">
                    <span class="title">VIP Users of Alcolock:</span>
                ...
                <?php   if(!empty($_SERVER["QUERY_STRING"])){
                        $pre_id=$_SERVER["QUERY_STRING"];
                        $array;
                        parse_str($pre_id,$array);
                        }
                        $counter=1;
                        $query="SELECT * FROM tb_users_207";
                        $result=mysqli_query($connection, $query);
                        while($row=mysqli_fetch_assoc($result)){
                            if($row["admin"]!=1&&$row["VIP"]==1){
...
<label>enter coupon id to add:<input type='number' name='couponid' minlength='1' value=''></label><br>
<label>enter drinker id to added to:<input type='number' name='drinkerid' minlength='1' value=''></label><br>
<label><input type='submit' value='add coupon'></label>
</form>";

if(!empty($array["couponid"])&&!empty($array["drinkerid"])){

$sql = "INSERT INTO `studDB19a`.`tb_users_207_usersXcoupons` (`drinker_id`, `coupon_id`) VALUES (".$array["drinkerid"].", ".$array["couponid"].");";
...
?>
    ...
    <script src="includes/VIP/VIPJS.js"></script>
</div>
</body>
